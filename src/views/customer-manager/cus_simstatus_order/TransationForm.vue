<style scoped>
  ...
  #transationTable tr th:last-child,#cmccTable tr td:last-child{border-right:1px solid #dcdcdc;}
      ...
      <Row v-if="transationDataList.length>0">
        <table id="transationTable">
          ...
          <tr v-for="item in transationDataList ">
            <td>{{item.msisdn}}</td>
            <td>{{item.resultDesc=='SUCCESS'?'成功':item.resultDesc}}</td>
